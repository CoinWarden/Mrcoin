...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog* dialog)
{
    ...
    int nDisplayUnit = MrcoinUnits::BTC;
    ...
    l2->setText(MrcoinUnits::formatWithUnit(nDisplayUnit, nAmount));        // Amount
    l3->setText(MrcoinUnits::formatWithUnit(nDisplayUnit, nPayFee));        // Fee
    l4->setText(MrcoinUnits::formatWithUnit(nDisplayUnit, nAfterFee));      // After Fee
    l5->setText(((nBytes > 0) ? "~" : "") + QString::number(nBytes));        // Bytes
    l6->setText(sPriorityLabel);                                             // Priority
    l7->setText((fLowOutput ? (fDust ? tr("Dust") : tr("yes")) : tr("no"))); // Low Output / Dust
    l8->setText(MrcoinUnits::formatWithUnit(nDisplayUnit, nChange));        // Change
    ...
    toolTip1 += tr("This means a fee of at least %1 per kB is required.").arg(MrcoinUnits::formatWithUnit(nDisplayUnit, CTransaction::nMinTxFee)) + "<br /><br />";
    toolTip1 += tr("Can vary +/- 1 byte per input.");

    QString toolTip2 = tr("Transactions with higher priority are more likely to get included into a block.") + "<br /><br />";
    toolTip2 += tr("This label turns red, if the priority is smaller than \"medium\".") + "<br /><br />";
    toolTip2 += tr("This means a fee of at least %1 per kB is required.").arg(MrcoinUnits::formatWithUnit(nDisplayUnit, CTransaction::nMinTxFee));

    QString toolTip3 = tr("This label turns red, if any recipient receives an amount smaller than %1.").arg(MrcoinUnits::formatWithUnit(nDisplayUnit, CENT)) + "<br /><br />";
    toolTip3 += tr("This means a fee of at least %1 is required.").arg(MrcoinUnits::formatWithUnit(nDisplayUnit, CTransaction::nMinTxFee)) + "<br /><br />";
    toolTip3 += tr("Amounts below 0.546 times the minimum relay fee are shown as dust.");

    QString toolTip4 = tr("This label turns red, if the change is smaller than %1.").arg(MrcoinUnits::formatWithUnit(nDisplayUnit, CENT)) + "<br /><br />";
    toolTip4 += tr("This means a fee of at least %1 is required.").arg(MrcoinUnits::formatWithUnit(nDisplayUnit, CTransaction::nMinTxFee));
...
void CoinControlDialog::updateView()
{
    ...
    BOOST_FOREACH(PAIRTYPE(QString, vector<COutput>) coins, mapCoins)
    {
        ...
        BOOST_FOREACH(const COutput& out, coins.second)
        {
            ...
            {
                sAddress = CMrcoinAddress(outputAddress).ToString().c_str();

                // if listMode or change => show Mrcoin address. In tree mode, address is not shown again for direct wallet address outputs
            ...
            // amount
            itemOutput->setText(COLUMN_AMOUNT, MrcoinUnits::format(nDisplayUnit, out.tx->vout[out.i].nValue));
        ...
        if (treeMode)
        {
            ...
            itemWalletAddress->setText(COLUMN_AMOUNT, MrcoinUnits::format(nDisplayUnit, nSum));
